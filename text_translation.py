...
#pipe = pipeline("translation", model="Helsinki-NLP/opus-mt-zh-en",device=1)
...
#json_test_data=json.load(open('/home/poong/tjfwownd/PosterNUWA/data/cgl_dataset/layout_train_6w_translated.json'))
...
for i,annotations in enumerate(json_data['annotations']):
    ...
    for j,annot in enumerate(annotations):
        if annot['category_id']==2:
            ...
            #translation_result=pipe(ocrs)
            print(translation_result)
            #translation_result = [trans['translation_text'] for trans in translation_result]
