...
def metrics_uti(img_names, clses, boxes):
    ...
    for idx, name in enumerate(img_names):
        pic_1 = np.array(Image.open(os.path.join("/data1/poong/tjfwownd/PosterNUWA/data/cgl_dataset/radm_dataset/RADM_dataset/images/test_PFPN", 
                                               name.replace("jpg","png"))).convert("L").resize((513, 750))) / 255
        pic_2 = np.array(Image.open(os.path.join("/data1/poong/tjfwownd/PosterNUWA/data/cgl_dataset/radm_dataset/RADM_dataset/images/test_BasNet", 
                                               name.replace("jpg","png"))).convert("L").resize((513, 750))) / 255
...
def metrics_rea(img_names, clses, boxes):
    ...
    metrics = 0
    for idx, name in enumerate(img_names):
        pic = Image.open(os.path.join("/data1/poong/tjfwownd/PosterNUWA/data/cgl_dataset/radm_dataset/RADM_dataset/images/test", name.replace("png","png"))).convert("RGB").resize((513, 750))
...
def main():
    ...
    log_dir = "/data1/poong/tjfwownd/PosterNUWA/log_dir/Paper_samples/Generated_Sample_RADM_test/DS_GAN"
    print(log_dir)
