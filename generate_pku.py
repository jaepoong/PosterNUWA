...
# TODO customizing
def main(
    file_path: str = "data/PKU_PosterLayout/for_posternuwa/html_format_img_instruct_all_mask_and_all_condition/test_numerical.jsonl",
    base_model: str = "log_dir/train_stage2_without_augment_dino_codellama_pku/checkpoints/checkpoint-16/pytorch_model.bin",
    device: int=1,
    output_dir: str="log_dir/train_stage2_without_augment_dino_codellama_pku",
    checkpoint: str="16",
    ...
    with tqdm(total=len(content)) as pbar:
        for i,samples in enumerate(content):
            try:
                img_path = os.path.join("data/PKU_PosterLayout/train/inpainted_poster",samples['name'][0][:-4]+".png")
                image = Image.open(img_path)
            except:
                img_path = os.path.join("data/PKU_PosterLayout/train/inpainted_poster",samples['name'][0][0][:-4]+".png")
                image = Image.open(img_path)  
            ...
            generated_sample = evaluate(img_l,instruct, max_new_tokens=max_new_tokens,do_sample=do_sample)
