...
def compute_iou(
    ...
    # intersection
    l_max = lib.maximum(l1, l2) # 왼쪽 최대
    r_min = lib.minimum(r1, r2) # 오른쪽 최소
    t_max = lib.maximum(t1, t2) # 위쪽 최대
    b_min = lib.minimum(b1, b2) # 아래쪽 최소
    cond = (l_max < r_min) & (t_max < b_min) #
    ai = lib.where(cond, (r_min - l_max) * (b_min - t_max), lib.zeros_like(a1[0])) # 겹치면 ai, 아니면 0

    au = a1 + a2 - ai # 전체 volume
    iou = ai / au # 겹치는 부분 / 전체 volume

    if not generalized:
        return iou

    # outer region
    l_min = lib.minimum(l1, l2)  # 왼쪽 최소
    r_max = lib.maximum(r1, r2)  # 오른쪽 최대
    t_min = lib.minimum(t1, t2)  # 취쪽 최소
    b_max = lib.maximum(b1, b2)  # 아랫쪽 최대
    ac = (r_max - l_min) * (b_max - t_min)

    giou = iou - (ac - au) / ac # iou - (가장 바깥 넓이 - 겹치는 넓이)/가장 바깥 넓이.
    # 즉 겹치지 않는 부분도 고려.
