...
def metrics_uti(img_names, clses, boxes):
    ...
    for idx, name in enumerate(img_names):
        pic_1 = np.array(Image.open(os.path.join("/data1/poong/tjfwownd/desigen/data/processed_images/PFPN", 
                                               name.replace("jpg","png"))).convert("L").resize((513, 750))) / 255
        pic_2 = np.array(Image.open(os.path.join("/data1/poong/tjfwownd/desigen/data/processed_images/BasNet", 
                                               name.replace("jpg","png"))).convert("L").resize((513, 750))) / 255
...
def metrics_rea(img_names, clses, boxes):
    ...
    metrics = 0
    for idx, name in enumerate(img_names):
        pic = Image.open(os.path.join("/data1/poong/tjfwownd/desigen/data/processed_images/images", name.replace("png","jpg"))).convert("RGB").resize((513, 750))
...
def main():
    ...
    log_dir = "/data1/poong/tjfwownd/PosterNUWA/log_dir/Paper_samples/Generated_Sample_desigen/LayoutPrompter/CGL/4"
    print(log_dir)
